...
exports.getTopFiveGames = function (callback) {
    DButilsAzure.Select(dBConnection, 'SELECT Top 5 dbo.GamesInOrders.gameName ,picPath, sum(dbo.GamesInOrders.amount) AS Total FROM Orders LEFT JOIN (dbo.GamesInOrders left join [dbo].[Games] on GamesInOrders.gameName = Games.gameName) ON dbo.Orders.orderID = dbo.GamesInOrders.orderID WHERE DATEDIFF(day,current_timestamp,orderDate) <= 7 GROUP BY dbo.GamesInOrders.gameName ,picPath ORDER BY Total desc ',
        callback)
};
...
exports.getLastMonthItems = function (callback) {
    DButilsAzure.Select(dBConnection, 'select  gameName,picPath from Games where DATEDIFF(day,current_timestamp,releaseDate) <= 30 ',
        callback)
};
...
exports.getUserByID = function (userName, callback) {
    ...
    DButilsAzure.Select(connection, 'select * from [Clients] where userName = ' + "'" + userName + "'", callback)
};
...
exports.getRecomandation = function (userName, callback) {
    DButilsAzure.Select(dBConnection, 'select top 5 part1.gameName,picPath,publisher,price, ISNULL(Total,0) as Quantity from (select gameName , picPath,publisher,price from (ClientCategories as A inner join [dbo].[Games] as B on A.category = B.category ) where A.userName = ' + "'" + userName + "'" + ') as part1 left join (select gameName , sum(amount) as Total from [dbo].[GamesInOrders] group by gameName) as part2 on part1.gameName = part2.gameName order by Total desc'
        , callback)
};
...
exports.login = function (userName, password, callback) {
    let connection = new Connection(dBconfig);
    DButilsAzure.promiseSelect(connection, 'select * from [Clients] where userName = ' + "'" + userName + "' and password = " + "'" + password + "'")
        .then(function (ans) {
            if (ans.length > 0) {
                ...
                let result = {
                    ...
                    type: type,
                    lastTime: lastTime,
                    formattedString: 'The ' + type + " " + userName + ' is connected, last login: ' + lastTime,
                    success: true
                };
                callback(result);
            }
...
let updateLastLogin = function (userName, callback) {
    let connection = new Connection(dBconfig);
    DButilsAzure.Update(connection, 'UPDATE Clients SET lastLogin = (CURRENT_TIMESTAMP) where userName = ' + "'" + userName + "'",function (ans){
            callback(ans);
        })
};
